...
fn solve(grid: &Vec<Vec<char>>, start: [usize; 2], end: [usize; 2]) -> HashSet<[usize; 2]> {
    ...
    let mut optimal_path = HashSet::new();
    ...
    while attempts.len() > 0 {
        ...
        if current_pos == end {
            ...
            current_attempt.iter().for_each(|x| {
                optimal_path.insert(*x);
            });
            continue;
        }
        ...
        for neighbor in neighbors {
            ...
            if score == visited_scores[neighbor[0]][neighbor[1]] {
                optimal_path.insert(neighbor);
                attempts.push((Reverse(score), new_attempt));
                continue;
            }
            if score
                > visited_scores[neighbor[0]][neighbor[1]]
                    .checked_add(1000)
                    .unwrap_or(i32::MAX)
            {
                continue;
...
fn print_map(grid: &Vec<Vec<char>>, path: HashSet<[usize; 2]>) {
    for (row, line) in grid.iter().enumerate() {
        for (col, c) in line.iter().enumerate() {
