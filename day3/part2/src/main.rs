fn main() {
    ...
    let mut do_indices: Vec<usize> = input.match_indices("do()").map(|x| x.0).collect();
    do_indices.insert(0, 0);
    println!("{:?}", do_indices);

    let dont_indices: Vec<usize> = input.match_indices("don't()").map(|x| x.0).collect();
    println!("{:?}", dont_indices);

    let mut sum = 0;

    for mul_index in mul_indices {
        if !do_do(mul_index, do_indices.clone(), dont_indices.clone()) {
            continue;
        }
...
fn do_do(mul_index: usize, do_indices: Vec<usize>, dont_indices: Vec<usize>) -> bool {
    let mut greatest_do_index: usize = 0;
    for do_index in do_indices {
        if do_index > mul_index {
            break;
        }
        greatest_do_index = do_index;
    }
    let mut greatest_dont_index: usize = 0;
    for dont_index in dont_indices {
        if dont_index > mul_index {
            break;
        }
        greatest_dont_index = dont_index;
    }
    greatest_do_index >= greatest_dont_index
}
